...
if [ $XDEBUG -eq 1 ]; then
  COMMAND="$PHPUNIT -d memory_limit=-1 -c ./tests $@ --configuration phpunit.xml --coverage-html ./build/coverage"
else
  COMMAND="$PHPUNIT -v -c ./tests $@ --configuration phpunit.xml --coverage-html ./build/coverage"
fi
